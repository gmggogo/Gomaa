...
function extractionPrompt(template,ocrText=""){
  ...
  const lines = [
    ...
    "SEARCH SCOPE IS STRICT: extract ONLY the Document fields explicitly supplied below. Ignore every other word, label, header, footer, logo, company name, instruction, invoice/reference number, side note, signature, disclaimer, or unrelated text on the page.",
    "Do not create a requested field from unrelated text elsewhere on the page just because the text looks semantically similar.",
    "If a requested Document field is not visibly present, or its cell is blank, return an empty string for that field.",
    "Use the VISIBLE DOCUMENT HEADER and the physical table/grid position above each cell. Do not use GH Mobility internal field names to decide column ownership.",
    ...
    "CELL BOUNDARIES ARE AUTHORITATIVE: determine which column a handwritten value belongs to from the center/bulk of the handwriting inside the visible grid cell. If handwriting touches or crosses a vertical line, keep it with the cell containing most of the writing. Never duplicate it into the neighboring cell.",
    "Never move text from Stops to Dropoff Address, from Dropoff Address to Stops, or between any adjacent columns merely because its meaning looks more appropriate.",
    "If ownership between two neighboring cells is genuinely ambiguous, leave the uncertain requested field blank rather than borrowing text from its neighbor.",
    "Example: if the visible header is Pickup Date, the value physically under it must be returned under Pickup Date. If the visible header is Pickup Time, return it under Pickup Time.",
    ...
    "For a Stops field, preserve only text visibly written inside the Stops cell/column; separate multiple stops within that same cell with semicolons.",
    "ADDRESS READING: for Pickup Address, Stops, and Dropoff Address, use the original image as the authority. You may correct an obvious letter-level handwriting/OCR mistake only when the intended spelling is clear from the visible writing and immediate address context.",
    "Never invent, guess, autocomplete, or add a house number, street name, city, state, ZIP code, direction, or suffix that is not supported by the visible cell.",
    "Do not use text from another cell to repair an address. If an address word is genuinely unreadable, preserve the best literal reading or leave the uncertain portion blank instead of hallucinating.",
