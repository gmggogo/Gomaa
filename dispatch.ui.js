...
async function sendOne(tripId){
  ...
  if(!select || !select.value){
    showToast("اختار السواق الأول")
    return
  }

  try{
    const assignRes = await fetch(`/api/dispatch/${tripId}/driver`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ driverId: select.value })
    })

    const assignData = await assignRes.json().catch(()=>({}))

    if(!assignRes.ok){
      showToast(assignData?.message || "Driver assign failed")
      return
    }
    ...
    trip.driverId = select.value
    trip.driverName = assignData?.driverName || trip.driverName
    trip.vehicle = assignData?.vehicle || getDriverCar(select.value)
...
async function sendSelected(){
  ...
  try{
    ...
    for(const trip of selected){
      const select = document.getElementById(`driver-${trip._id}`)

      if(!select || !select.value){
        showToast("اختار السواق لكل الرحلات")
        return
      }

      const assignRes = await fetch(`/api/dispatch/${trip._id}/driver`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ driverId: select.value })
      })
