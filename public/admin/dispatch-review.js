...
function operationalServiceCode(value){
  ...
  /*
    CUSTOM_n is the immutable Platform identity, not the operational
    code shown on trips. Historical/current trips use the locked
    two-letter code such as ME.
  */
...
function getServiceTitleByTrip(t){
  ...
  /*
    Historical snapshot wins. Renaming a custom service later must never
    rewrite how an already-created trip is displayed in Review.
  */
  const snapshot =
    normalizeText(
      t?.serviceName ||
