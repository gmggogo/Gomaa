...
function operationalServiceCode(value){
  ...
  /*
    CUSTOM_n is the immutable Platform identity only.
    Trips and financial summaries use the operational two-letter code.
  */
...
function getServiceIdentityFromTrip(t){

  const stored =
    normalizeKnownCode(
      t?.summaryServiceIdentity ||
      t?.serviceIdentity ||
      ""
    );
  ...
  return getServiceCodeFromTrip(t);
}
...
function summaryServiceCatalog(){
...
function renderServiceFilter(){
  ...
  const catalog =
    summaryServiceCatalog();

  serviceFilter.innerHTML =
    catalog
      .map(item=>`
        <option value="${safe(item.code)}">${safe(item.title)}</option>
      `)
      .join("");
