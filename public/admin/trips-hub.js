...
/*
  ...
  show the near 30-day window immediately, then load the long-range
  reservation inbox in the background without blocking the page.
*/
let hubBackgroundLoading = false;
let hubBackgroundLoadedOnce = false;
let hubBackgroundLastLoadedAt = 0;

const HUB_BACKGROUND_REFRESH_MS =
  5 * 60 * 1000;
...
function getServiceCodeFromService(s){
  return normalizeKnownCode(
    s?.customServiceCode || s?.serviceCode || s?.serviceKey || s?.key || s?.code || s?.suffix || s?.companySuffix || s?.title || s?.name || ""
  );
}
...
function mergeHubTripLists(primary,secondary){
  const map =
    new Map();
...
async function fetchHubTripsStage(loadStage){
  ...
  const stagePart =
    loadStage
      ? `loadStage=${encodeURIComponent(loadStage)}&`
      : "";

  const res =
    await fetch(
      `${LIST_API_URL}${separator}${stagePart}_=${Date.now()}`,
      {
        cache:"no-store",
        headers:{
          ...(token ? {Authorization:"Bearer " + token} : {}),
  ...
  if(!res.ok){
    throw new Error(
      `Trips Hub request failed (${res.status})`
    );
  }
...
async function loadHubBackgroundTrips(
  force = false
){

  if(hubBackgroundLoading){
    return;
  }
  ...
  if(
    !force &&
    hubBackgroundLoadedOnce &&
    (
      now -
      hubBackgroundLastLoadedAt
    ) < HUB_BACKGROUND_REFRESH_MS
  ){
    return;
  }

  hubBackgroundLoading = true;

  try{

    const backgroundTrips =
      await fetchHubTripsStage(
        "background"
      );

    /*
      Replace only the long-range portion.
      Keep the current near-window rows that may have just refreshed.
    */
    const currentNearTrips =
      hubTrips.filter(
        isNearHubTrip
      );
    ...
    hubBackgroundLoadedOnce = true;
    hubBackgroundLastLoadedAt =
      Date.now();

    renderLoadedHubTrips();

    /*
      Old overdue active rows can still be normalized, but this happens
      after the visible page is already rendered.
    */
    autoMarkNotCompleted(
      backgroundTrips
    )
      .then(()=>{
        if(!editingKey){
      ...
      .catch(err=>{
        console.log(
          "Background Not Completed update failed",
          err
        );
      });
    ...
    console.log(
      "Trips Hub background load failed",
      err
    );

  }finally{

    hubBackgroundLoading = false;
...
async function loadHubTrips(){
  ...
  try{
    ...
    /*
      First request is intentionally small:
      yesterday through the next 30 days.
    */
    const nearTrips =
      await fetchHubTripsStage(
        "initial"
    ...
    /*
      If the long-range background has already loaded, preserve it while
      replacing the near window with fresh rows. This also lets deleted
      near-window trips disappear correctly on refresh.
    */
    const existingBackgroundTrips =
      hubBackgroundLoadedOnce
        ? hubTrips.filter(
            trip=>
              !isNearHubTrip(trip)
    ...
    /*
      Do NOT await this.
      Trips Hub is already visible while the remaining months load.
    */
    loadHubBackgroundTrips(
      !hubBackgroundLoadedOnce
    );
    ...
    /*
      ...
      if the staged endpoint is not available for any reason,
      try the original full endpoint once.
    */
    if(!hubTripsLoadedOnce){
      ...
      try{
        ...
        console.log(
          "Trips Hub fallback load failed",
          fallbackErr
        );
