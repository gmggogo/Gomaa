...
(() => {
  ...
  function normalizeServiceCode(value){
    ...
    /*
      Custom services arrive from the server with their locked
      two-letter operational code, for example ME.
      Never collapse an arbitrary custom code into a core service.
    */
    return raw;
  }
    ...
    return catalog.map(
      rawCatalog=>{
        ...
        return {
          ...
          /*
            Saved Broker Pricing values win over catalog defaults,
            while identity/name come from the current dynamic catalog.
          */
          enabled:
          ...
          baseFare:
            num(existing.baseFare),

          includedMiles:
            num(
              existing.includedMiles
            ),

          perMile:
            num(existing.perMile),

          hourlyRate:
            num(existing.hourlyRate),

          hourlyBillingMode:
            upper(
              existing.hourlyBillingMode ||
              "FULL"
            ) === "QUARTER"
              ? "QUARTER"
              : "FULL",
          ...
          stopFee:
            num(existing.stopFee),

          noShowFee:
            num(existing.noShowFee),

          sharedPrice:
            shared
              ? num(
                  existing.sharedPrice
                )
              : 0,
          ...
          warningMinutes:
            num(
              existing.warningMinutes
            ),

          cancelFee:
            num(existing.cancelFee),
