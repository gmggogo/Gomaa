...
(() => {
  ...
  function itemServiceCode(item){

    const candidates = [
      item?.serviceCode,
      item?.serviceKey,
      item?.serviceSuffix,
      item?.tripNumberSuffix,
      item?.serviceType
    ];
      ...
      if(!map.has(code)){
        ...
        map.set(
          code,
          {
            ...
            serviceIdentity:
              clean(
                item?.serviceIdentity
              ),
            customServiceSlot:
              Number(
                item?.customServiceSlot ||
                0
              )
          }
        );
  ...
  function renderServiceFilter(){
    ...
    select.innerHTML =
      ...
      state.services
        .map(service=>`
          <option value="${safe(service.code)}">
            ${safe(service.name || service.code)}
          </option>
        `)
        .join("");
