(function () {
  ...
  async function refreshAll(){
    ...
    const panelOpen =
      document
        .getElementById("sbChatPanel")
        ?.classList.contains("open") === true;

    /*
      LIGHT BACKGROUND MODE:
      - Keep only unread badge fresh while the chat is closed.
      - Online driver list is loaded only when the panel is open.
      - Messages are loaded only when a conversation is open.
      This removes repeated Mongo work from every admin page.
    */
    if(!panelOpen){
      await loadUnread();
      return;
  ...
  async function init(){
    ...
    /*
      ...
      Only unread count is requested; driver list waits until chat opens.
    */
    await loadUnread();
