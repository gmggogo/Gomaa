...
const platformSupportRoutes =
  require("./routes/platformSupportRoutes");
...
.then(async () => {
  ...
  /*
    Archive warmup runs in the background.
    It uses the program timezone from System Design and never blocks startup.
    Persistent cutoffDate controls exactly which finished days are missing.
  */
  startArchiveWarmupWorker({
    fallbackTimeZone:
      SYSTEM_TIMEZONE ||
    ...
    warmAdminArchive:
      warmAdminSummaryArchivesAllTenants,

    warmCompanyArchives:
      warmCompanySummaryArchivesAllTenants
  });
...
/* =========================
   TRIP MODEL
   Extracted to models/Trip.js; schema/index behavior is unchanged.
========================= */
...
/* =========================
   ARCHIVE WARMUP - ALL TENANTS

   The worker calls these once after startup and then only when the
   program date changes. Each archive's own cutoffDate prevents old work
   from being recalculated.
========================= */

async function archiveWarmupTenantIds(){
  const rows =
    await Tenant.find({
      enabled:{ $ne:false }
    })
...
async function warmAdminSummaryArchivesAllTenants({
  todayKey,
  timeZone
} = {}){
  if(
  ...
  for(const tenantId of tenantIds){
    try{
      ...
      if(
        ...
        console.log(
          "ADMIN SUMMARY WARMUP:",
          tenantId,
          result.action,
          result.previousCutoff || "",
          "->",
          result.cutoffDate || todayKey
        );
      }
      ...
      console.log(
        "ADMIN SUMMARY TENANT WARMUP ERROR:",
        tenantId,
        err?.message || err
      );
    }
  ...
  if(errors > 0){
    throw new Error(
      `Admin Summary warmup failed for ${errors} tenant(s)`
    );
  }
}

async function warmCompanySummaryArchivesAllTenants({
  todayKey,
  timeZone
} = {}){
  const tenantIds =
  ...
  for(const tenantId of tenantIds){
    try{
      const result =
        await warmCompanySummaryArchiveForTenant({
          tenantId,
          todayKey
        });
      ...
      if(
        result &&
        (
          result.initialBuilds > 0 ||
          result.appended > 0 ||
          result.errors > 0
        )
      ){
        console.log(
          "COMPANY SUMMARY WARMUP:",
          tenantId,
          {
            companies:result.companies,
            current:result.current,
            initialBuilds:
              result.initialBuilds,
      ...
      console.log(
        "COMPANY SUMMARY TENANT WARMUP ERROR:",
        tenantId,
        err?.message || err
      );
    }
  ...
  if(errors > 0){
    throw new Error(
      `Company Summary warmup failed for ${errors} company/tenant operation(s)`
    );
  }
}
...
function tripServiceSuffix(value){
...
function tripServiceLegacyVehicle(code){
...
async function resolveTripCreateService({
  tenant,
  tenantId,
  requestedService,
  isShared=false
  ...
  /*
    Backward compatibility for old tenants:
    the six original services can continue to work if the Service
    document has not yet been bootstrapped. Custom services never
    bypass Platform Admin's CUSTOM_n master gate.
  */
  if(!service){
    ...
    if(
      ...
      throw Object.assign(
        new Error(
          "Custom service is not configured for this organization"
        ),
        { statusCode:409 }
      );
    }
    ...
    return {
      service:null,
      gateKey:coreCode,
      operationalCode:coreCode,
      displayName:
        tripServiceLegacyVehicle(
          coreCode
        ),
  ...
  if(
    ...
    throw Object.assign(
      new Error(
        "Custom service is not configured"
      ),
      { statusCode:409 }
    );
  }
  ...
  if(!identity.gateKey){
    throw Object.assign(
      new Error(
        "Unable to resolve service identity"
      ),
      { statusCode:409 }
    );
  }
  ...
  if(
    ...
    throw Object.assign(
      new Error(
        "Service code is not configured"
      ),
      { statusCode:409 }
    );
  }
...
/* =========================
   ...
   Platform Admin permission uses the immutable core/CUSTOM_n gate.
   Trip numbers and pricing use the operational two-letter code.
========================= */

const resolvedTripService =
  await resolveTripCreateService({
    tenant,
    tenantId,
    requestedService:
      rawVehicle,
...
app.get(
  ...
  async (req,res)=>{
    ...
    try{
      ...
      /*
        Trips Hub staged loading:
        - initial: yesterday through the next 30 days, so the page can render fast.
        - background: everything outside that near window.
        - no loadStage: preserve the original full endpoint behavior for compatibility.
      */
      const loadStage =
        String(
          req.query?.loadStage ||
      ...
      if(
        ...
        function stagedTripDateKey(offsetDays = 0){
...
/* =========================
   ADMIN TRIPS - TODAY + TOMORROW ONLY

   Trips Hub keeps using /api/tenant-trips for its long-range reservation inbox.
   Admin Trips uses this endpoint because that page only needs today + tomorrow.
========================= */
app.get(
  "/api/tenant-trips/today-tomorrow",
  requireTenantApi,
  async (req,res)=>{
    ...
    try{
      ...
      /*
        Build date keys in the same configured system timezone used by Admin Trips.
        This avoids UTC day-boundary errors.
      */
      ...
      console.log(
        "ADMIN TODAY TOMORROW TRIPS ERROR:",
        err
      );

      return res.status(500).json({
        message:
          "Error loading admin trips"
      });
...
/* =========================
   COMPANY SUMMARY - PERSISTENT HISTORY ARCHIVE

   PERFORMANCE DESIGN:
   - Historical closed company trips are calculated once and saved in Mongo.
   - Normal page opens read history from company_summary_archive.
   - Only today's/new closed trips are recalculated on refresh.
   - On a new day, only the newly-finished day is appended to the archive.
   - Existing pricing/status/output shape is preserved.
========================= */

const CompanySummaryArchive =
  mongoose.models.CompanySummaryArchive ||
  mongoose.model(
    "CompanySummaryArchive",
    new mongoose.Schema(
      {
        tenantKey:{
          type:String,
      ...
      {
        timestamps:true,
        collection:"company_summary_archive"
      }
    ).index(
      {
        tenantKey:1,
...
const CompanySummaryArchiveState =
  mongoose.models.CompanySummaryArchiveState ||
  mongoose.model(
    "CompanySummaryArchiveState",
    new mongoose.Schema(
      {
        tenantKey:{
          type:String,
      ...
      {
        timestamps:true,
        collection:"company_summary_archive_state"
      }
    ).index(
      {
        tenantKey:1,
...
const companySummaryHistoryMemory =
  new Map();

const companySummaryResponseCache =
  new Map();

const COMPANY_SUMMARY_CACHE_TTL_MS =
  10 * 1000;

function companySummaryDateKey(
  date = new Date()
){
  const parts =
    new Intl.DateTimeFormat(
...
function companySummaryTenantKey(req){
...
function companySummaryCompanyKey(
  companyName
){
  return String(
    companyName ||
...
function companySummaryMemoryKey(
  req,
  companyName
){
  return [
    companySummaryTenantKey(req),
    companySummaryCompanyKey(
      companyName
    )
  ].join("::");
}

async function companySummaryQueryTrips(
  req,
  companyName,
  rangeFilter = {}
){
  ...
  /*
    Summary displays only final/closed rows.
    This also excludes scheduled/open trips at Mongo level.
  */
  filter.status = {
...
async function buildCompanySummaryRows(
  trips
){
...
async function upsertCompanySummaryRows(
  req,
  companyName,
  rows
){
...
async function rebuildCompanySummaryHistory(
  req,
  companyName,
  todayKey
){
  ...
  const historicalTrips =
    await companySummaryQueryTrips(
      req,
      companyName,
      {
        tripDate:{
  ...
  await upsertCompanySummaryRows(
    req,
    companyName,
    rows
  );

  await CompanySummaryArchiveState
    .findOneAndUpdate(
      {
        tenantKey,
        companyKey
  ...
  companySummaryHistoryMemory.set(
    companySummaryMemoryKey(
      req,
      companyName
    ),
    {
...
async function syncCompanySummaryHistory(
  req,
  companyName,
  todayKey
){
  ...
  const memory =
    companySummaryHistoryMemory.get(
      memoryKey
    );
  ...
  if(!state){
    return rebuildCompanySummaryHistory(
      req,
      companyName,
      todayKey
    );
...
/* =========================
   COMPANY SUMMARY BACKGROUND WARMUP

   Called only by archiveWarmupWorker.
   Existing cutoffDate is authoritative:
   - current cutoff => no old Trip scan
   - old cutoff     => append only missing finished days
   - no state       => one-time historical build
========================= */

async function warmCompanySummaryArchiveForTenant({
  tenantId,
  todayKey
} = {}){
  const cleanTenantId =
  ...
  for(const company of companies){
    ...
    try{
      ...
      /* Never delete/rebuild completed archive days during warmup. */
      if(
        cutoff &&
        cutoff > effectiveTodayKey
      ){
        result.skippedFutureCutoff += 1;
        continue;
      }

      if(!cutoff){
        await rebuildCompanySummaryHistory(
          req,
          companyName,
          effectiveTodayKey
        );

        companySummaryResponseCache.delete(
          companySummaryMemoryKey(
            req,
            companyName
          )
      ...
      companySummaryHistoryMemory.delete(
        memoryKey
      );
...
app.get(
  ...
  async (req,res)=>{
    ...
    try{
      ...
      const cached =
        companySummaryResponseCache.get(
          cacheKey
        );
      ...
      /*
        ...
        LIVE only checks today/future closed rows.
      */
      const [
        historicalRows,
        liveTrips
        ...
        await Promise.all([
          syncCompanySummaryHistory(
            req,
            companyName,
            todayKey
          ),

          companySummaryQueryTrips(
            req,
            companyName,
            {
              tripDate:{
      ...
      companySummaryResponseCache.set(
        cacheKey,
        {
          at:Date.now(),
          rows:result
      ...
      if(
        companySummaryResponseCache.size >
        100
      ){
        const oldestKey =
          companySummaryResponseCache
            .keys()
            .next()
            .value;
...
/* =========================
   COMPANY DASHBOARD - FAST LIVE + ARCHIVE

   Dashboard needs:
   - every trip for TODAY, including Scheduled / Confirmed / active rows
   - current-month historical closed rows
   - no full company-history scan on each refresh

   Historical rows reuse the persistent Company Summary archive.
   Today's rows stay live.
========================= */

const companyDashboardCache =
  new Map();

const COMPANY_DASHBOARD_CACHE_TTL_MS =
  10 * 1000;

function companyDashboardMonthStartKey(
  todayKey
){
  const text =
    String(todayKey || "");
...
async function companyDashboardTodayTrips(
  req,
  companyName,
  todayKey
){
  ...
  /*
    Company Dashboard is for the normal company flow.
    Keep broker records out of this page.
  */
  filter.$and = [
    {
      $or:[
  ...
  async (req,res)=>{
    ...
    try{
      ...
      if(
        ...
        COMPANY_DASHBOARD_CACHE_TTL_MS
      ){
        return res.json(
          cached.payload
        );
      ...
      /*
        Run both reads together:
        1) persistent historical Summary archive
        2) today's live Trip rows (all statuses)
      */
      const [
        historicalRows,
        todayTripsRaw
      ...
      /*
        Keep the current month only from the archive.
        Old months never need to be sent to Dashboard.
      */
      const monthHistory =
      ...
      /*
        Preserve the exact Summary pricing/status behavior for closed rows
        that happened today, while keeping open rows live as raw trip data.
      */
      const todayClosedRaw =
        todayTripsRaw.filter(row=>
          /cancel|no|complete/i.test(
            String(
              row?.status ||
              ""
            )
      ...
      const payload = {
        ...
        requestStats:{
          ...
          liveWindow:"TODAY",
          historyWindow:"CURRENT_MONTH",
          fullHistoryScan:false
        }
      };

      companyDashboardCache.set(
        cacheKey,
        {
          at:Date.now(),
          payload
      ...
      if(
        companyDashboardCache.size >
        100
      ){
        const oldestKey =
          companyDashboardCache
            .keys()
            .next()
            .value;

        if(oldestKey){
          companyDashboardCache.delete(
            oldestKey
          );
        }
      }
      ...
      return res.status(500).json({
        success:false,
        message:
          "Failed loading Company Dashboard"
      });
    }
  }
);
