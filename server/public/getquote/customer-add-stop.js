/* =========================================
FILE: customer-add-stop.js
CUSTOMER GET QUOTE ADD STOP
Secure email-link route editor
Frontend only - server routes added separately
========================================= */
...
/*
  SERVER ENDPOINTS WE WILL CREATE:
...
const API_CUSTOMER_TRIP = token =>
  `/api/customer-add-stop/${encodeURIComponent(token)}`;

const API_CUSTOMER_CONFIRM = token =>
  `/api/customer-add-stop/${encodeURIComponent(token)}/confirm`;
...
const form =
  document.getElementById("addStopForm");
...
const pageStatusBadge =
  document.getElementById("pageStatusBadge");

const tripNumberInput =
  document.getElementById("tripNumber");
...
(function injectStyle(){
  ...
  const old =
    document.getElementById(
      "customer-add-stop-dynamic-style"
    );
...
async function fetchCustomerTrip(){

  if(!accessToken){

    throw new Error(
      "This Add Stop link is missing or invalid"
    );
  }

  const res =
    await fetch(
      API_CUSTOMER_TRIP(accessToken),
      {
        method:"GET",
        cache:"no-store",
        headers:{
  ...
  if(
    ...
    throw new Error(
      data.message ||
      "This Add Stop link is invalid or expired"
    );
  }
    ...
    new Promise(
      async (resolve,reject)=>{
        ...
        try{
          ...
          script.src =
            `https://maps.googleapis.com/maps/api/js?key=${encodeURIComponent(data.googleKey)}`;
...
function hideOldControls(){
  ...
  if(addStopBtn){
    addStopBtn.classList.add(
      "old-add-stop-hidden"
    );
  }

  if(confirmAddStopBtn){
    confirmAddStopBtn.classList.add(
      "old-add-stop-hidden"
    );
  }
}
...
function getConfirmedNewStopsForSlot(
  slotIndex
){
...
function confirmNewStop(id){
  ...
  if(!address){
    ...
    showAlert(
      "error",
      "Please enter the stop address first"
    );
  ...
  if(
    ...
    showAlert(
      "error",
      "This address is already in the route"
    );
...
function startExistingEdit(index){
  ...
  setTimeout(()=>{
    ...
    const input =
      document.querySelector(
        `.existing-edit-input[data-index="${index}"]`
      );
...
function confirmExistingEdit(index){
  ...
  if(!value){
    ...
    showAlert(
      "error",
      "Existing stop address cannot be empty"
    );
...
function buildFinalStops(
  originalExistingStops,
  addedStopObjects
){
  ...
  for(
    let anchorIndex = 0;
    anchorIndex <= editedStops.length;
    anchorIndex++
  ){
...
function buildOriginalRouteBeforeStart(
  trip
){
...
function buildNewRouteBeforeStart(
  trip,
  finalStops,
  dropoffAfter
){
...
async function calculateFinalRouteChange(
  ...
  if(inProgress){
    ...
    if(!driverLocationAtConfirm){
      ...
      throw new Error(
        "Driver current location is unavailable. Please contact the transportation company."
      );
    }
...
async function notifyServerAddStop(
  payload
){

  const res =
    await fetch(
      API_CUSTOMER_CONFIRM(
        accessToken
      ),
      {
        method:"POST",
  ...
  if(
    ...
    throw new Error(
      data.message ||
      "The new route or price could not be approved"
    );
  }
...
async function finalSubmitAddStop(){
  ...
  try{
    ...
    if(newStopDrafts.length){
      ...
      throw new Error(
        "You have an unconfirmed stop field. Confirm it or remove it first."
      );
    }
    ...
    setGlobalLoading(
      true,
      "Confirming new price with your bank..."
    );
    ...
    const payload = {
      ...
      existingStopsBefore:
        calc.existingStopsBefore,

      editedExistingStops:
        calc.editedExistingStops,

      addedStops:
        calc.addedStops,

      addedStopsDetailed:
        calc.addedStopsDetailed,

      finalStops:
        calc.finalStops,

      finalRoutePoints:
        calc.finalRoutePoints,
      ...
      beforeStopChange:{
        ...
        priceAmount:
          Number(
            freshTrip.priceAmount || 0
          ),

        finalPrice:
          Number(
            freshTrip.finalPrice || 0
          )
      },
    ...
    showAlert(
      "error",
      err.message ||
      "The new route was not applied. Your original route remains active."
    );
...
function fillPage(trip){
  ...
  if(hasActiveStopRequest(trip)){
    ...
    showAlert(
      "info",
      "This trip already has an active route change request. Contact the transportation company before submitting another request."
    );
  }
...
document.addEventListener(
  ...
  e=>{
    ...
    if(
      action ===
      "remove-confirmed-new-stop"
    ){

      removeConfirmedNewStop(
        btn.dataset.id || ""
      );
...
async function init(){
  ...
  try{
    ...
    showAlert(
      "error",
      err.message ||
      "Failed to load this Add Stop page"
    );
