...
async function init(){
  try{
    ...
    // ✅ فقط الرحلات المختارة للديسبتش وغير المعطلة
    trips = tripsRaw
...
function isDriverActiveOnDate(driverId, tripDate){
  ...
  // لو اليوم غير موجود خالص، ما نقفلش السواق بالغلط
  if(!(key in days)) return true
...
function getSmartStartAddress(driver, currentTrip){
  ...
  // ✅ نجيب آخر رحلة قبل الرحلة الحالية فقط
...
async function autoAssign(){
  ...
  // ابدأ بعداد مؤقت من الرحلات المانول الحالية
  trips.forEach(t => {
    if(t.manual && t.driverId){
      tempCount[String(t.driverId)] = (tempCount[String(t.driverId)] || 0) + 1
    }
  })

  // امسح التعيين الأوتوماتيك فقط
  trips.forEach(t => {
    if(!t.manual){
      t.driverId = ""
      t.vehicle = ""
  ...
  for(const trip of trips){
    ...
    for(const driver of validDrivers){
      ...
      const startAddress = getSmartStartAddress(driver, trip)
      const startPoint =
        normalizeText(startAddress) === normalizeText(getDriverDayHomeAddress(driver))
          ? driver._geoHome
      ...
      // ✅ هل هيلحق من آخر رحلة للرحلة الجديدة؟
      let timingPenalty = 0
      const prevAssignedTrips = trips
        .filter(t =>
          String(t.driverId || "") === String(driver._id) &&
          String(t._id || "") !== String(trip._id || "")
        )
      ...
      for(const t of prevAssignedTrips){
        const ts = getTripDateTimeValue(t)
        if(ts && tripTs && ts < tripTs){
          prevTrip = t
        }
      }

      if(prevTrip){
        const prevTs = getTripDateTimeValue(prevTrip)
        ...
        // 20 دقيقة buffer فوق وقت الحركة التقريبي
      ...
      // ✅ العدالة
      const loadPenalty = (tempCount[driver._id] || 0) * 500

      // ✅ boost لو العنوان شبه بعض
      let boost = 0
    ...
    if(bestDriver){
      ...
      tempCount[bestDriver._id] = (tempCount[bestDriver._id] || 0) + 1
    }
  }
...
// ✅ عشان لو الـ HTML القديم بينادي Engine.*
window.Engine = {
  toggleSelect,
  toggleEdit,
  sendSelected,
