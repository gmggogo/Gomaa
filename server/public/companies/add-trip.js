...
async function loadCompanyServices(){
  ...
  try{
    ...
    const overrideRes =
      await fetch(
        "/api/facility-pricing/resolve?" + params.toString(),
        {
          headers:{
            Authorization:"Bearer " + token
          }
    ...
    console.log(
      "ADD TRIP FACILITY OVERRIDE RESULT:",
      overrideData
    );
    ...
    if(
      ...
      console.log(
        "ADD TRIP SERVICES FROM FACILITY OVERRIDE:",
        COMPANY_SERVICES
      );
    ...
    console.warn(
      "ADD TRIP NO FACILITY OVERRIDE. FALLBACK SERVICE MANAGEMENT.",
      overrideData
    );
