...
function externalReviewTripPayload(
  ...
  return {
    ...
    serviceIdentity:
      clean(
        externalTrip.serviceIdentity
      ) ||
      serviceKey,
    customServiceSlot:
      Number(
        externalTrip.customServiceSlot ||
        0
      ),
    serviceSuffix:serviceKey,
    tripNumberSuffix:serviceKey,
...
function serviceCatalogRow(service){
...
async function tenantExternalServiceCatalog(
  tenantId
){
...
async function resolveTenantExternalService(
  tenantId,
  input
){
...
router.get(
  ...
  async (req,res) => {
    ...
    try{
      ...
      const services =
        await tenantExternalServiceCatalog(
          req.authUser.tenantId
        );
      ...
      for(const key of allowed){
        ...
        if(key === "serviceKey"){
          ...
          const resolvedService =
            await resolveTenantExternalService(
              req.authUser.tenantId,
              req.body.serviceKey
            );
          ...
          if(!resolvedService){
            ...
            return res.status(403).json({
              success:false,
              message:
                "Service is not enabled or configured for this organization"
            });
          }
