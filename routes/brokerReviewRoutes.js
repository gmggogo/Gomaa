...
async function resolveBrokerReviewService({
  tenantId,
  externalTrip,
  shared,
  session
  ...
  if(!input){
    throw new Error(
      "Broker trip service is missing"
    );
  }
  ...
  /*
    ...
    old broker rows for the six built-in services may predate tenant
    Service documents. Do not break those historical trips.
  */
  if(!service){
    ...
    throw new Error(
      "Broker custom service is not configured for this company: " +
      input
    );
  }
  ...
  if(
    ...
    throw new Error(
      "Broker service is not enabled for this company"
    );
  }
          ...
          for(const trip of trips){
            ...
            */
            ...
            /*
              ...
              Resolve against the tenant's Service Management definition
              before Broker Review releases the trip to Dispatch.

              - Shared group => SH
              - Individual core service => ST/WH/LM/TX/XL
              - Individual custom service => its locked two-letter code
                such as ME, while its Platform identity remains CUSTOM_n.

              Trip Split must not overwrite the original broker service.
            */
            const resolvedService =
              await resolveBrokerReviewService({
                tenantId,
                externalTrip:
                  primaryExternal,
                shared,
            ...
            if(!finalService){
              throw new Error(
                "Unable to resolve broker service"
              );
            }
            ...
            /*
              Snapshot the immutable Platform identity when the Trip schema
              supports these fields. Existing schemas safely ignore unknown
              paths, while serviceKey/serviceCode remain the operational code.
            */
            trip.serviceIdentity =
              resolvedService
                .gateKey ||
