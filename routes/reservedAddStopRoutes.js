...
function isValidCode(code){
  ...
  return ["ST","WH","XL","LM","TX","SH"].includes(normalized) || /^[A-Z]{2}$/.test(normalized);
}
...
function serviceCode(service){
  return normalizeCode(
    service?.customServiceCode || service?.serviceCode || service?.serviceKey || service?.serviceType ||
