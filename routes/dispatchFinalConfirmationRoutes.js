    ...
    router.get("/", requireTenantApi, async (req,res)=>{
      ...
      try{
        ...
        /*
          FINAL CONFIRMATION FAST READ PATH V2

          Performance rules:
          - keep exact final statuses so Mongo can use indexes
          - push the 12-hour confirmed visibility window into Mongo
          - avoid loading old confirmed trips that Node would immediately discard
          - keep positive projection
          - keep lean()
          - no ExternalTrip query on page load
          - GET remains read-only
          - sort only after the reduced result set is returned

          Confirm / Payment / Shared Confirm / Return-to-Driver routes below
        ...
        */
        ...
        const holdCutoff =
          new Date(
            Date.now() -
            (HOLD_HOURS * 60 * 60 * 1000)
          );

        /*
          A row is visible when:
          1) it has a final status (trip or shared passenger), AND
          2) it is still unconfirmed, OR it was confirmed within HOLD_HOURS.

          This mirrors the existing page helpers, but prevents Mongo from
          returning years of already-confirmed historical trips.
        */
          ...
          $or:[
            {
              $and:[
                {
                  $or:[
                    { finalStatusConfirmed:{ $ne:true } },
                    { finalStatusConfirmed:{ $exists:false } }
                  ]
                },
                {
                  $or:[
                    { sharedFinalConfirmed:{ $ne:true } },
                    { sharedFinalConfirmed:{ $exists:false } }
                  ]
                },
                {
                  $or:[
                    { dispatchFinalConfirmedAt:null },
                    { dispatchFinalConfirmedAt:{ $exists:false } }
                  ]
                },
                {
                  $or:[
                    { finalStatusConfirmedAt:null },
                    { finalStatusConfirmedAt:{ $exists:false } }
                  ]
                },
                {
                  $or:[
                    { sharedFinalConfirmedAt:null },
                    { sharedFinalConfirmedAt:{ $exists:false } }
                  ]
                }
              ]
            },
            { dispatchFinalConfirmedAt:{ $gte:holdCutoff } },
            { finalStatusConfirmedAt:{ $gte:holdCutoff } },
            { sharedFinalConfirmedAt:{ $gte:holdCutoff } }
          ]
        };
            ...
            $and:[
              {
                $or:[
                  ...
                  { "passengers.status":{ $in:finalStatuses } }
                ]
              },
              visibilityFilter
            ]
        ...
        /*
          Preserve the old visible order without forcing Mongo to perform
          a potentially expensive cross-branch sort on the $or query.
        */
        result.sort((a,b)=>{
