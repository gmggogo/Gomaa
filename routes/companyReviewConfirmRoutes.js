...
/* =========================
   COMPANY REVIEW - OPEN TRIPS ONLY

   PERFORMANCE:
   Company Review never displays completed / cancelled / no-show history.
   Do not download the company's whole Trip history every 30 seconds.
========================= */
...
router.get(
  ...
  async (req,res)=>{
    ...
    try{
      ...
      /*
        Normal company users are always resolved from their authenticated
        account. Never trust a browser-supplied company name for tenant users.
      */
      if(role !== "platform_admin"){
        ...
        const actor =
          ...
          .select(
            "name company companyName facilityName organizationName username"
          )
          .lean();
      ...
      const filter =
        tenantFilter(
          req,
          {
            ...
            /*
              Exactly mirrors the frontend's hidden-history intent:
              Completed / Not Completed / Cancelled / No Show rows are not
              Review work items.
            */
            $and:[
              {
                status:{
                  $not:
                    /complete|cancel|noshow|no[\s-]*show/i
                }
              },
              {
                status:{
              ...
              /* normal company flow only, not broker history */
              {
                $or:[
      ...
      console.log(
        "COMPANY REVIEW OPEN TRIPS ERROR:",
        err
      );
      ...
      return res.status(500).json({
        success:false,
        message:
          "Failed loading Company Review trips"
      });
    }
  }
);
...
function buildServiceSearchFilter(idOrKey){
  ...
  return {
    $or:[
      ...
      {serviceCode:key},{serviceCode:rawUpper},{customServiceCode:key},{customServiceCode:rawUpper},
