...
/*
  Bootstrap coordinate repair used to wait for every trip one-by-one.
  Keep the exact same coordinate function/save behavior, but process a
  small bounded batch at a time so Trip Split opens faster without
  flooding Google/geocoding or MongoDB.
*/
async function ensureBootstrapCoordinates(
  trips,
  batchSize = 4
){
  const rows = safeArray(trips);
...
router.get("/bootstrap",async(req,res)=>{
  try{
    ...
    /*
      ...
      This first query exists only so ensureReturnTrips() can create missing
      return legs. Loading every open broker trip here duplicated the main
      bootstrap query unnecessarily. Only rows that actually carry returnTime
      can create a return leg.
    */
    const returnSourceTrips =
      await ExternalTrip.find({
        tenantId,
    ...
    */
    await ensureBootstrapCoordinates(
      trips
    );
