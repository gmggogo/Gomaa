...
function code(v){
  ...
  /*
    Custom Services are dispatched by their locked two-letter operational
    code (for example ME). Never turn CUSTOM_1 into a driver service code.
  */
  if(
    serviceIdentity
      .isCustomGate(
...
function serviceRequirement(trip,ctx){
  const key = tripService(trip);
  if(!key || !(ctx?.serviceRequirements instanceof Map)) return null;
  return ctx.serviceRequirements.get(key) || null;
}

function capabilityAllows(row,trip,ctx){

  const req = serviceRequirement(trip,ctx);
  if(!req) return true;

  const neededVehicle = clean(req.vehicleCategory).toUpperCase();
  const driverVehicle = clean(row?.vehicleCategory || "GENERIC").toUpperCase();

  if(
    neededVehicle &&
    neededVehicle !== "GENERIC" &&
    driverVehicle !== neededVehicle
  ){
    return false;
  }

  if(req.requiresCDL === true){
    if(row?.cdlLicensed !== true) return false;

    const neededClass = clean(req.cdlClass).toUpperCase();
    if(
      neededClass &&
      cdlRank(row?.cdlClass) < cdlRank(neededClass)
    ){
      return false;
    }
  }
  ...
  if(
    num(req.minimumVehicleCapacityLb,0) > 0 &&
    num(row?.vehicleCapacityLb,0) <
      num(req.minimumVehicleCapacityLb,0)
  ){
    return false;
  }

  if(req.requiresLiftGate === true && row?.liftGate !== true) return false;
  if(req.refrigeratedRequired === true && row?.refrigerated !== true) return false;
  if(req.hazmatRequired === true && row?.hazmatEndorsed !== true) return false;
...
async function buildContext(req){
  ...
  const [drivers,rows,settings,tenantTrips,serviceRows] = await Promise.all([
    User.find(
    ...
    .select(
      "serviceKey serviceCode customServiceCode companySuffix reservedSuffix serviceSuffix " +
      "vehicleCategory requiresCDL cdlClass minimumVehicleCapacityLb requiresLiftGate refrigeratedRequired hazmatRequired"
    )
    .lean()
  ]);
  ...
  for(const service of serviceRows){
    const operational =
      serviceIdentity.getServiceOperationalCode(service);

    if(!operational) continue;

    serviceRequirements.set(operational,{
      vehicleCategory:clean(service.vehicleCategory || "GENERIC").toUpperCase(),
      requiresCDL:service.requiresCDL === true,
      cdlClass:clean(service.cdlClass).toUpperCase(),
      minimumVehicleCapacityLb:Math.max(0,num(service.minimumVehicleCapacityLb,0)),
...
function rejectionReason(trip,ctx){
  ...
  const capabilityRows =
    serviceRows.filter(
      item => capabilityAllows(item.row,trip,ctx)
    );

  if(!capabilityRows.length){
    return `No driver has the required vehicle / CDL capability for ${required || "this service"}`;
  }

  const scheduleRows = capabilityRows.filter(item=>
...
function rankDrivers(trip,ctx){
  ...
  return ctx.drivers.flatMap(driver=>{
    ...
    */
    ...
    if(!capabilityAllows(row,trip,ctx)){
      return [];
    }
