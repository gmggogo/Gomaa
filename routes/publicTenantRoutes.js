...
/* =========================================
   PUBLIC RESERVED BOOKING DATA
   Read-only configuration for Reserved page.

   Reads ONLY matrix.reserved:
   - showField
   - showColumn
   - showEye
   - required

   Does NOT change pricing, routing, Confirm,
   cancellation, Add Stop, or button policy.
========================================= */

function publicReservedBookingFields(config){
  ...
  function isReservedSelected(rule){
    return (
      rule?.showField === true ||
  ...
  function reservedStandardLabel(item){
  ...
  function reservedStandardType(item){
  ...
  const standardFields =
    standard
      .filter(item=>{
        ...
        const rule =
          item?.matrix?.reserved;

        return (
          item &&
          clean(item?.key) &&
          isReservedSelected(rule)
        );
      })
      .map((item,index)=>{
        ...
        return {
          ...
          label:
            reservedStandardLabel(item),

          fieldType:
            reservedStandardType(item),
...
async function sendPublicReservedBookingData(
  res,
  tenant
){
...
router.get(
  ...
  async (req,res)=>{
    ...
    try{
      ...
      return await sendPublicReservedBookingData(
        res,
        tenant
      );
      ...
      console.error(
        "PUBLIC DEFAULT RESERVED BOOKING DATA ERROR:",
        err
      );
      ...
      return res.status(500).json({
        success:false,
        message:
          "Failed To Load Reserved Booking Data"
      });
    }
  }
);
...
router.get(
  ...
  async (req,res)=>{
    ...
    try{
      ...
      console.error(
        "PUBLIC TENANT RESERVED BOOKING DATA ERROR:",
        err
      );
