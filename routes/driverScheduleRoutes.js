  ...
  try{
    ...
    items.forEach(item=>{
      ...
      result[item.driverId] = {
        ...
        vehicleCategory:item.vehicleCategory || "GENERIC",
        cdlLicensed:item.cdlLicensed === true,
        cdlClass:item.cdlClass || "",
        vehicleCapacityLb:Number(item.vehicleCapacityLb || 0),
        liftGate:item.liftGate === true,
        refrigerated:item.refrigerated === true,
        hazmatEndorsed:item.hazmatEndorsed === true,
      ...
      await DriverSchedule.findOneAndUpdate(
...

        {
          $set:{
            ...
            vehicleCategory:
              String(data.vehicleCategory || "GENERIC").trim().toUpperCase(),

            cdlLicensed:data.cdlLicensed === true,

            cdlClass:
              data.cdlLicensed === true
                ? String(data.cdlClass || "").trim().toUpperCase()
                : "",

            vehicleCapacityLb:
              Math.max(0,Number(data.vehicleCapacityLb || 0) || 0),

            liftGate:data.liftGate === true,
            refrigerated:data.refrigerated === true,
            hazmatEndorsed:data.hazmatEndorsed === true,
